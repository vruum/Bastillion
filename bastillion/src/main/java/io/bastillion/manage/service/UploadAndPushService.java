...
public class UploadAndPushService extends BaseKontroller {

    public static final String UPLOAD_PATH = DBUtils.class.getClassLoader().getResource(".").getPath() + "../upload";
    private static Logger log = LoggerFactory.getLogger(UploadAndPushService.class);
    ...
    public UploadAndPushService(HttpServletRequest request, HttpServletResponse response) {
    ...
    public String push() {
        ...
        try {
            ...
            if (pendingSystemStatus != null) {
                ...
                for (Integer instanceId : SecureShellService.getUserSchSessionMap().get(sessionId).getSchSessionMap().keySet()) {

                    //if host system id matches pending system then upload
                    if (pendingSystemStatus.getId().equals(SecureShellService.getUserSchSessionMap().get(sessionId).getSchSessionMap().get(instanceId).getHostSystem().getId())) {
                        session = SecureShellService.getUserSchSessionMap().get(sessionId).getSchSessionMap().get(instanceId);
                    }
                }
